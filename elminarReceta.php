...
<html lang="es">
  ...
  <body>
    ...
    <h4 class="center">Elimina Recetas</h4>

    <div class="section container">
        <form action="eliminarRecetas.php" method="post" enctype="multipart/form-data">
         <div class="row">
            
    <div class="input-field col s11 m6">
        <select name="id" required>
          <option disabled selected>Seleccione Receta</option>
       <?php 
                    mysqli_data_seek($result,0);
                    while ($row=mysqli_fetch_assoc($result)){ 
            ?>
                <OPTION value="<?php echo $row['idReceta'] ?>"><?php echo $row['titulo']?></OPTION>
			     
            <?php 
                    }
        ?>
        </select>
        <label class="black-text" for="ite">Recetas Registrados</label>
      </div>
